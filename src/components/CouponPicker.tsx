      ...
      <div className="space-y-2">
        <div className="flex items-center gap-3 rounded-xl border-2 border-emerald-500 bg-emerald-50 dark:bg-emerald-950/30 px-3 py-2.5">
          ...
          <button
            ...
            className="shrink-0 text-xs font-bold text-sky-400 bg-white border-2 border-sky-300 hover:bg-sky-50 rounded-full px-3 py-1.5 transition-colors"
          >
        ...
        {expanded && (
          <div className="rounded-2xl bg-gradient-to-br from-orange-100 to-orange-200/80 dark:from-orange-950/40 dark:to-orange-900/30 p-3 shadow-sm">
            <p className="text-sm font-extrabold text-orange-700 dark:text-orange-300 leading-tight tracking-tight mb-1">
              Escolha outro CUPOM
            </p>
            <p className="text-[11px] text-orange-900/80 dark:text-orange-200/80 leading-snug mb-2.5">
              Toque no cupom atual para remover, ou selecione outro
            </p>
              ...
              {list.map((c) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "min-w-0 rounded-xl border-2 px-2 py-2 flex flex-col items-start gap-0.5 transition-all text-left motion-reduce:animate-none",
                      isCurrent
                        ? "border-emerald-500 bg-emerald-500"
                        : eligible
                          ? "border-transparent bg-white dark:bg-background animate-coupon-pulse hover:[animation-play-state:paused] hover:scale-[1.07] hover:border-orange-400 hover:shadow-md active:scale-[0.98] cursor-pointer"
                          : "border-dashed border-border bg-white/60 dark:bg-background/60 opacity-60 cursor-not-allowed",
                    )}
                  ...
                  >
                    ...
                    >
                      {isCurrent ? "Atual · toque p/ remover" : `${c.min_quantity} conjuntos · R$${Math.round(c.bundle_price)}`}
                    </span>
                  </button>
                );
              })}
